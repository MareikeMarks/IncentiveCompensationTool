...
**Purpose:** Track quarter-over-quarter GMV growth to identify scaling merchants
...
SELECT 
    ...
    -- Project Q1 based on current pace
    CASE 
...
**Returns:**
- Q4 2025 GMV (baseline quarter)
- Q1 2026 GMV year-to-date
- Q1 2026 GMV projected (extrapolated based on current pace)

**Notes:**
- GMV growth = leading indicator for future revenue growth
- High GMV growth + low product adoption = opportunity
...
**Purpose:** Identify which revenue-driving products each merchant has enabled

**Tables:**
- `shopify-dw.money_products.shopify_payments_adoption_current`
- `shopify-dw.pos.pos_locations_and_devices_snapshot`
- `shopify-dw.international.markets_shops`
- `shopify-dw.b2b.b2b_company_locations`
- `shopify-dw.capital.capital_advances`
...
WITH adoption_data AS (
    ...
    LEFT JOIN `shopify-dw.money_products.shopify_payments_adoption_current` sp 
    ...
    LEFT JOIN (
        SELECT DISTINCT shop_id 
        FROM `shopify-dw.b2b.b2b_company_locations`
    ) b2b ON s.shop_id = b2b.shop_id
    LEFT JOIN (
        SELECT shop_id, TRUE AS has_active_loan
        FROM `shopify-dw.capital.capital_advances`
...
**Returns:**
- Boolean flags for each product: SP, POS, Markets, B2B, Capital
- Missing products = upsell opportunities

**Use Case:**
- Identify merchants with high GMV but missing key products
- Prioritize outreach: "You have $50M GMV but no POS"

---

## Query 10: Recent Product Enablements
**Purpose:** Track recent product launches to measure impact and share success stories

**Tables:**
- `shopify-dw.money_products.shopify_payments_adoption_history`
- `shopify-dw.pos.pos_locations_and_devices_snapshot`
- `shopify-dw.international.markets_shops`
...
WITH sp_enablements AS (
    SELECT 
        shop_id,
        'Shopify Payments' AS product_name,
        first_enabled_at AS enabled_date
    FROM `shopify-dw.money_products.shopify_payments_adoption_history`
    WHERE shop_id IN (${shopIds})
        AND first_enabled_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH)
),
pos_enablements AS (
    SELECT 
        shop_id,
        'POS' AS product_name,
        MIN(created_at) AS enabled_date
    FROM `shopify-dw.pos.pos_locations_and_devices_snapshot`
    WHERE shop_id IN (${shopIds})
...
markets_enablements AS (
    SELECT 
        shop_id,
        'Markets' AS product_name,
        created_at AS enabled_date
    FROM `shopify-dw.international.markets_shops`
    WHERE shop_id IN (${shopIds})
        AND created_at >= DATE_SUB(CURRENT_DATE(), INTERVAL 6 MONTH)
)
...
UNION ALL
SELECT * FROM pos_enablements
UNION ALL
SELECT * FROM markets_enablements
ORDER BY enabled_date DESC
```

**Returns:**
- Shop ID, product name, enablement date for recent launches
- Tracks last 6 months of product adoption activity

**Use Case:**
- Timeline of your recent wins
- Measure pre/post revenue impact (future enhancement)
